...
for currMeasNr = 1:nrOfEpochs
    ...
    if(bInit)
     ...
     if allSettings.sys.ls_type == 2
         % Option 2: Now finally we calculate the actual navigation solution with EKF (!!!) % written by Zhengdao
         [obsData, satData, navData, allSettings] = getNavSolution_EKF(obsData, satData, navData, allSettings, currMeasNr);   
     else
         % Option 1: Now finally we calculate the actual navigation solution
